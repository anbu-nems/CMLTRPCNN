...
def load_model_and_scalers():
    import pickle, json as _json, pandas as _pd
    ck = torch.load(MODEL_PT, map_location="cpu", weights_only=False)
    hp = ck["hp"]
    with open(os.path.join(ROOT, "model_weights", "cmltrv77_scalers.pkl"), "rb") as _fh:
        sc = pickle.load(_fh)
    sc_lst, sc_tilt, sc_res = sc["sc_lst"], sc["sc_tilt"], sc["sc_res"]
    lst_cols = sc["lst_cols"]
    n_lst, n_tilt, n_res = len(sc["lst_cols"]), len(sc["tilt_cols"]), len(sc["res_cols"])
    models = []
    for state in ck["models"]:
        m = CMLTRPCNNv71(
            ...
            dropout=0.0)
        m.load_state_dict(state, strict=False); m.eval().to(DEVICE)
        models.append(m)
    regime_names = ["regime_Ia", "regime_Ib", "regime_II", "regime_III"]
    regime_idx = [lst_cols.index(r) for r in regime_names if r in lst_cols]
    _df = _pd.read_parquet(os.path.join(PROC_DIR, "feature_matrix_v7.parquet"))
    _gii = _df["GII"].fillna(0.0).values.astype(np.float32) if "GII" in _df.columns else np.zeros(len(_df), np.float32)
    _tr = np.array(_json.load(open(os.path.join(PROC_DIR, "calibration_split_idx.json")))["train_idx"])
    _giit = _gii[_tr]
    gii_max = float(np.percentile(_giit[_giit > 0], 95) if (_giit > 0).sum() > 10 else 1.0)
    return models, sc_lst, sc_tilt, sc_res, gii_max, regime_idx
...
def main():
    ...
    if os.path.exists(s42_path):
        ...
        if mean_lofo >= 0.80 and cf_strong:
            final = "GENERALIZABLE MECHANISM — full publication-quality claim"
        elif mean_lofo >= 0.60 and cf_strong:
            final = "ROBUST TREND WITH LIMITED EXTRAPOLATION — strong publication-quality claim"
        elif mean_lofo >= 0.60:
            final = "ROBUST TREND — publication-quality claim with caveated extrapolation"
        elif cf_strong:
            final = "WITHIN-FAMILY MECHANISTIC TREND — npj CM or focused publication"
        else:
            final = "NOT publication-ready — strengthen LOFO and counterfactual"
