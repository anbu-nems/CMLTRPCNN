...
# Patched: point to PIML directory layout + import from local model_code
...
ROOT     = PIML_ROOT
PROC_DIR = os.path.join(PIML_ROOT, "data", "processed")
RES_DIR  = os.path.join(RELEASE_ROOT, "extracted_data"); os.makedirs(RES_DIR, exist_ok=True)
FIG_DIR  = os.path.join(RELEASE_ROOT, "all_figures");  os.makedirs(FIG_DIR, exist_ok=True)
MODEL_PT = os.path.join(PIML_ROOT, "model_weights", "cmltrv77_final.pt")
CSV_OUT  = os.path.join(RES_DIR, "lofo_predictions_v77.csv")
_LOFO_RECORDS = []
LOG_FLOOR = 5.0
...
def load_regime_idx():
    ...
    part = _json.load(open(os.path.join(PROC_DIR, "feature_partition_v7.json")))
    lst_cols = part["LST"]
    regime_names = ["regime_Ia", "regime_Ib", "regime_II", "regime_III"]
    return [lst_cols.index(r) for r in regime_names if r in lst_cols]
...
def lofo_run(label, train_idx, test_idx, Xl, Xt, Xr, er_cm, has_cm,
    ...
    # Patched: capture per-sample predictions for CSV export
    for j, i in enumerate(test_idx):
        _LOFO_RECORDS.append({
            "label":        label,
            "idx":          int(i),
...
def main():
    ...
    # Patched: save per-sample LOFO predictions to CSV
    pd.DataFrame(_LOFO_RECORDS).to_csv(CSV_OUT, index=False)
    print(f"  Per-sample CSV → {CSV_OUT}  ({len(_LOFO_RECORDS)} rows)")
    print(f"  Results → {RES_DIR}/42_lofo_generality.json")
