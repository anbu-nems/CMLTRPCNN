...
"""
Script 48 — Final retrain: best HP from run_hp_tuning.py (100-trial Optuna, cleaned dataset)

HP source: results/hp_tuning_optuna.db Trial 97 (verified R²=0.9397 ± 0.0165, 3 seeds)
Key differences vs previous HP (Trial 34 on raw data):
  trunk_hidden   : 512 → 384   (smaller trunk generalises better on n=1165)
  n_trunk_blocks :   2 → 1     (one ResBlock sufficient)
  tilt_hidden    :  32 → 64    (tilt branch needs more capacity)
  res_hidden     : 192 → 256   (residual branch benefits from more width)
  residual_scale :  80 → 44.9  (less over-correction needed on clean data)
  lr             : 1.72e-3 → 1.11e-3
  log_loss_alpha : 0.757 → 0.505  (more balanced log/MSE)
  lambda_vca     : 0.009 → 0.045  (stronger VCA penalty)
  n_seeds        : 5 (same)
  n_epochs       : 500 (same)

Outputs:
  model_weights/cmltrv77_final.pt
...
ROOT      = os.environ.get("PIML_ROOT", RELEASE_ROOT)
PROC_DIR  = os.path.join(ROOT, "data", "processed")
MODEL_OUT = os.path.join(ROOT, "model_weights", "cmltrv77_final.pt")
...
# Best HP from run_hp_tuning.py — 100-trial Optuna on cleaned dataset, Trial 97
HP = {
    "trunk_hidden":      384,
    "n_trunk_blocks":    1,
    "lst_hidden":        96,
...
def main():
    ...
    Xl_s, Xt_s, Xr_s, gii_s, sc_lst, sc_tilt, sc_res = scale_arrays(Xl, Xt, Xr, gii, train_idx)
    ...
    # Save scalers for downstream scripts
    import pickle as _pkl
    scalers_path = os.path.join(os.path.dirname(MODEL_OUT), "cmltrv77_scalers.pkl")
    with open(scalers_path, "wb") as _f:
        _pkl.dump({"sc_lst": sc_lst, "sc_tilt": sc_tilt, "sc_res": sc_res}, _f)
    print(f"  Scalers saved → {scalers_path}")

    # Save holdout predictions CSV for conformal calibration
    pred_holdout = predict(final_models, Xl_s, Xt_s, Xr_s, er_cm, has_cm, cm_approx,
                           gii_s, phase_tr, calib_idx, regime_idx)
    r2_holdout  = float(r2_score(y[calib_idx], pred_holdout))
    mae_holdout = float(np.mean(np.abs(y[calib_idx] - pred_holdout)))
    print(f"  Holdout R²={r2_holdout:.4f}  MAE={mae_holdout:.2f}  (n={len(calib_idx)})")
    import pandas as _pd
    holdout_df = _pd.DataFrame({
        "er_measured":  y[calib_idx],
        "er_predicted": pred_holdout,
    })
    holdout_path = os.path.join(PROC_DIR, "test_holdout_predictions.csv")
