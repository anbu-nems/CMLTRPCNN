...
class CMLTRPCNN(nn.Module):
    """
    CM-LTR Physics-Constrained Neural Network — final version CMLTRPCNNv7.7.

    Architecture (Optuna-tuned, 80 trials):
      Trunk:  512 hidden units, 2 residual blocks (pre-norm SiLU, skip connections)
      LST branch:  96 hidden units  (Softplus output → δ_LST ≥ 0)
      Tilt branch: 32 hidden units  (Softplus output → δ_tilt ≤ 0)
      Residual:   192 hidden units  (Tanh output, confidence-gated)
      residual_scale = 80.0

    Physics:
      εr = εr_CM + δ_LST(≥0) + δ_tilt(≤0) + σ_conf × tanh(res) × scale
      Fallback MLP activates when CM not available (has_cm = 0).
    """
...
# cmltrv77_final.pt). Old code/pickles that reference CMLTRPCNN keep working.
CMLTRPCNN = CMLTRPCNN

# ── Backward-compatibility alias ────────────────────────────────────────────
# Architecture froze at v7.1; published model is CMLTRPCNN v7.7 (cmltrv77_final.pt).
