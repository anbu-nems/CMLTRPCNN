...
# figS1 reads the bundled CSV directly; no feature_engineering import required



OUT_DIR  = os.path.join(RELEASE_ROOT, "supplementary")
FM_PATH  = os.path.join(RELEASE_ROOT, "data/processed/feature_matrix_v7.parquet")
CSV_PATH = os.path.join(RELEASE_ROOT, "data/raw/mixed_dataset_clean.csv")
...
for bar, val, pct in zip(bars2, asite_vals, asite_pct):
    ax_asite.text(bar.get_width() + 5, bar.get_y() + bar.get_height()/2,
                  f"{val}  ({pct:.0f}%)", va="center", fontsize=6.5, color="#333")
ax_asite.set_xlim(0, max(asite_vals) * 1.38)

# Note: Mixed = co-occupied A-site (La/Na/Bi/K and minor lanthanides)
ax_asite.text(0.97, 0.04, "Mixed = La/Na/Bi/K co-occupied",
              transform=ax_asite.transAxes, ha="right", va="bottom",
